...
const handleError = (res, message = 'Erro inesperado', status = 500, isAdmin, username, fotoExt) => {
...
router.get('/', verifyUser, async (req, res) => {
    ...
    if (role != 'admin' && role != 'docente') {
        return handleError(res, 'Role inválido.', 400, isAdmin, username, fotoExt);
    }
    ...
    try {
        ...
        res.render('listUsers', { users: response.data, title: realRole, role, isAdmin, username, fotoExt });
    } catch (e) {
        handleError(res, e.response.data.message, 501, isAdmin, username, fotoExt);
    }
});
...
router.post('/', upload.single('foto'), verifyUser, async (req, res) => {
    ...
    if (!req.body.username || !req.body.password || !req.body.password2 || !req.body.name || !req.body.email || !req.body.categoria || !req.body.filiacao) {
        if (req.file && req.file.path) fs.unlink(req.file.path, () => {});
        return res.render('register', { title: 'Registar', error: 'Preencha todos os campos.', role, isAdmin, username, fotoExt });
    }

    if (req.body.password !== req.body.password2) {
        if (req.file && req.file.path) fs.unlink(req.file.path, () => {});
        return res.render('register', { title: 'Registar', error: 'As passwords não coincidem.', role, isAdmin, username, fotoExt });
    }

    if (req.file.mimetype.split('/')[0] !== 'image') {
        if (req.file && req.file.path) fs.unlink(req.file.path, () => {});
        return res.render('register', { title: 'Registar', error: 'O ficheiro não é uma imagem.', role, isAdmin, username, fotoExt });
    }
    ...
    fs.access(oldPath, fs.constants.F_OK, (err) => {
        if (err) {
            return handleError(res, 'Uploaded file not found.', 500, isAdmin, username, fotoExt);
        }

        fs.mkdir(path.join(__dirname, '/../public/images/users/'), { recursive: true }, (err) => {
            if (err) {
                fs.unlink(oldPath, () => {});
                return handleError(res, 'Erro ao criar pasta', 500, isAdmin, username, fotoExt);
            }

            fs.rename(oldPath, newPath, async (err) => {
                if (err) {
                    fs.unlink(oldPath, () => {});
                    return handleError(res, 'Erro inesperado ao criar utilizador.', 500, isAdmin, username, fotoExt);
                }

                try {
                    const response = await axios.post(`${process.env.AUTH_URI}?role=${role}&token=${req.cookies.token}`, req.body);
                    if (response.data.error) {
                        return handleError(res, response.data.message, response.status, isAdmin, username, fotoExt);
                    }
                    ...
                    if (role === 'admin' || role === 'docente' || role === 'admindocente') {
                        ...
                        fs.mkdir(folderPath, { recursive: true }, (err) => {
                            if (err) {
                                fs.unlink(newPath, () => {});
                                return handleError(res, 'Erro ao criar pasta do utilizador.', 500, isAdmin, username, fotoExt);
                            }

                            const title = role.charAt(0).toUpperCase() + role.slice(1);
                            res.render('success', { title: 'Sucesso', sucesso: `${title} registado com sucesso.`, isAdmin, username, fotoExt });
                        });
                    } else {
                    ...
                    if (err.response && err.response.data.error.name == 'UserExistsError') {
                        fs.unlink(newPath, () => {});
                        return res.render('register', { title: 'Registar', error: 'Utilizador já existe.', role, isAdmin, username, fotoExt });
                    }
                    handleError(res, err, 500, isAdmin, username, fotoExt);
                }
            });
        });
    });
...
router.get('/delete', verifyUser, async (req, res) => {
    ...
    if (!req.query.username) {
        return handleError(res, 'Username inválido.', 400, isAdmin, username, fotoExt);
    }

    if (req.query.username === username) {
        return handleError(res, 'Não pode apagar a sua própria conta.', 403, isAdmin, username, fotoExt);
    }

    try {
        const response = await axios.delete(`${process.env.AUTH_URI}?username=${req.query.username}&token=${req.cookies.token}`);
        if (response.data.error) {
            return handleError(res, response.data.error, 500, isAdmin, username, fotoExt);
        }
        ...
        fs.rm(userFolderPath, { recursive: true }, (err) => {
            if (err) {
                return handleError(res, 'Erro ao apagar pasta', 500, isAdmin, username, fotoExt);
            }
            
            const userImagePath = path.join(__dirname, '/../public/images/users/', `${req.query.username}.${response.data.fotoExt}`);
            fs.unlink(userImagePath, (err) => {
                if (err) {
                    return handleError(res, 'Erro ao apagar ficheiro', 500, isAdmin, username, fotoExt);
                }
                        ...
                        dados.data.forEach(uc => {
                            if (uc.docentes.includes(req.query.username)) {
                                ...
                                axios.put(`${process.env.API_URI}/ucs/${uc._id}?token=${req.cookies.token}`, uc)
                                    .catch(e => handleError(res, 'Erro inesperado ao remover o docente das UCs', 500, isAdmin, username, fotoExt));
                            }
                        });
                        res.render('success', { title: 'Sucesso', sucesso: 'Utilizador apagado com sucesso.', isAdmin, username, fotoExt });
                    })
                    .catch(e => handleError(res, 'Erro inesperado ao remover o docente das UCs', 500, isAdmin, username, fotoExt));
            });
        });
    } catch (e) {
        handleError(res, e.response.data.message, 500, isAdmin, username, fotoExt);
    }
});
...
router.get('/registar', verifyUser, (req, res) => {
    ...
    res.render('register', { title: 'Registar', role, isAdmin, username, fotoExt });
});
